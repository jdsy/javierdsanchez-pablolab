...
if 1==1
  ...
  load('Z:\group\Topological Insulator\Samples\Thin Film Samples Ferhat\Alex_1\betterrack2.mat');
  %load('He4RackMag2Lockin');
  instrreset;
end
...
UseCryoMag = 1;
TwoLockin = 1; 
KeithleyVoltage = 0; %0 for current source (and T), 1 for voltage source (and gating) 
...
if KeithleyVoltage == 1
    ...
    fprintf(smdata.inst(k2400ind).data.inst,':sens:curr:prot 1e-5');
    fprintf(smdata.inst(k2400ind).data.inst,':sens:curr:RANG 1e-5');
...
%     fprintf(smdata.inst(3).data.inst,':sour:curr 0e-5')   % for current biasing
%     fprintf(smdata.inst(k2400ind).data.inst,':sour:curr:RANG 200e-6');
%     fprintf(smdata.inst(k2400ind).data.inst,':sens:volt:RANG 100e-3')
end
...
cd('Z:\group\Topological Insulator\Samples\Thin Film Samples Valla\ALD_Test\S4G4Y_ALD1');
